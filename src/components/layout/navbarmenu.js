...
const NavbarMenu = (props) => {
  ...
  const fetchApiFunc = async () => {
    try {
      const res = await navbarApi.getListNavbar();
      setNavbar(res.payload);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const elementNavbar = navbar.map((item, index) => (
    <li className="nav-item dropdown" key={index}>
      <a className="nav-link dropdown-toggle" href={item.path} id="navbarDropdownPortfolio" data-toggle="dropdown" aria-expanded="false">
        {item.name}
      </a>
      {/* <Route className="nav-link dropdown-toggle" path={item.path} id="navbarDropdownPortfolio" data-toggle="dropdown" aria-expanded="false">
        {item.name}
      </Route> */}
    </li>
  ));
